...
func (g *Generator) generateSchemaGoFile(outputPath, packageDir string, schemas []schemaInfo) error {
	...
	// Determine group and version from package directory
	group, version, err := determineGroupVersion(packageDir)
	if err != nil {
		return fmt.Errorf("failed to determine group version: %w", err)
	}
	...
	source.WriteString("\t\"github.com/thetechnick/orlop/pkg/apiserver/types\"\n")
	...
	for _, s := range schemas {
		...
		// Schema YAML constant
		source.WriteString(fmt.Sprintf("// %sSchemaYAML contains the OpenAPI v3 schema for %s.\n", s.typeName, s.typeName))
		source.WriteString(fmt.Sprintf("const %sSchemaYAML = `\n%s`\n\n", s.typeName, string(schemaYAML)))
	}
	...
	for _, s := range schemas {
		source.WriteString("\t\t{\n")
		source.WriteString(fmt.Sprintf("\t\t\tGVK: runtimeschema.GroupVersionKind{\n"))
		source.WriteString(fmt.Sprintf("\t\t\t\tGroup:   %q,\n", group))
